...
Графическая обёртка для [vopono](https://github.com/jamesmcm/vopono) — утилиты, которая запускает приложения внутри VPN-туннеля через отдельный network namespace. Работает с кастомными конфигами WireGuard без привязки к конкретному VPN-провайдеру.

## Что делает

- Позволяет выбрать WireGuard конфиг из любой директории
- Показывает установленные GUI-приложения с иконками, можно выбрать несколько
- Запускает выбранные приложения через vopono в одном namespace
- Запрашивает пароль графически (через `sudo -A` + askpass), терминал не нужен
- Сворачивается в системный трей при активном VPN-подключении
- Умеет чистить зависшие namespace после аварийного завершения
